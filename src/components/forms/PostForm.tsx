...
const PostForm = ({post, action}:PostFormProps) => {
 ...
 async function onSubmit(values: z.infer<typeof PostValidation>) {
  if(post && action === "Update"){
    ...
    if(!updatedPost){
      toast({
        title:'Plz try agin .Your post is not updated'
      })
    }
   ...
   if(!newPost){
    toast({
      title:'Your post is not created .Something went wrong'
    })
   }
   navigate('/')
  }
        ...
        />
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input className="shad-input" {...field} placeholder="Js, React Native, Next Js"/>
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
